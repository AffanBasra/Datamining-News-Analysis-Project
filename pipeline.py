...
from src.topic_modeling_word2vec import model_topics_word2vec
...
def run_pipeline(
    ...
    run_word2vec_topic_modeling=False,
    word2vec_model_type='cbow',
    run_ner=False,
    save_output=True
):
    """
    ...
    Args:
        ...
        run_topic_modeling (bool): If True, runs the BERTopic modeling step.
        run_word2vec_topic_modeling (bool): If True, runs the Word2Vec topic modeling step.
        word2vec_model_type (str): Type of Word2Vec model - 'cbow' or 'skipgram'.
    ...
    if run_topic_modeling: print("|--- OPTION ENABLED: BERTOPIC MODELING")
    if run_word2vec_topic_modeling: print(f"|--- OPTION ENABLED: WORD2VEC {word2vec_model_type.upper()} TOPIC MODELING")
    ...
    if run_topic_modeling:
        ...
        print("\nSkipping BERTopic modeling.")
        print("  To run this step, use the 'run_topic_modeling=True' argument.")

    # --- 3b. Conditional Word2Vec Topic Modeling ---
    if run_word2vec_topic_modeling:
        word2vec_topic_output_dir = Path("outputs/word2vec_topic_modeling")
        _, df = model_topics_word2vec(df, word2vec_topic_output_dir, model_type=word2vec_model_type) # We only need the df with topic assignments here
    else:
        print(f"\nSkipping Word2Vec {word2vec_model_type} topic modeling.")
        print("  To run this step, use the 'run_word2vec_topic_modeling=True' argument.")
    ...
    visualize_data(df, run_ner=run_ner)
    ...
    if run_topic_modeling: print(f"BERTopic outputs: {topic_model_output_dir}")
    if run_word2vec_topic_modeling: print(f"Word2Vec CBOW outputs: {word2vec_topic_output_dir}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--run_topic_modeling', action='store_true', help='Run the BERTopic modeling step.')
    parser.add_argument('--run_word2vec_topic_modeling', action='store_true', help='Run the Word2Vec topic modeling step.')
    parser.add_argument('--word2vec_model_type', type=str, default='cbow', choices=['cbow', 'skipgram'], help='Type of Word2Vec model to use: cbow or skipgram')
    ...
    run_pipeline(
        ...
        run_word2vec_topic_modeling=args.run_word2vec_topic_modeling,
        word2vec_model_type=args.word2vec_model_type,
